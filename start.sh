#!/usr/bin/env bash
# AtSubP2 web deployment manager.
set -Eeuo pipefail
...
check_health() {
    ...
    base_path="$(current_value NEXT_PUBLIC_BASE_PATH)"; base_path="${base_path:-/AtSubP2}"
    health_host="${bind_address}"; [[ "${health_host}" == 0.0.0.0 || "${health_host}" == :: ]] && health_host=127.0.0.1
    health_url="http://${health_host}:3210${base_path}"
    printf 'Waiting for %s ...\n' "${health_url}"
    for attempt in {1..30}; do
        if curl --fail --silent --show-error --max-time 5 "${health_url}" >/dev/null 2>&1; then printf 'AtSubP2 is ready at %s\n' "${health_url}"; return 0; fi
        sleep 2
    done
...
build_podman_app() {
    ...
    local build_args=(build --jobs=1 --pull --build-arg "NEXT_PUBLIC_BASE_PATH=$(current_value NEXT_PUBLIC_BASE_PATH)" --build-arg "NEXT_PUBLIC_TURNSTILE_SITE_KEY=$(current_value TURNSTILE_SITE_KEY)" --tag atsubp2-web:2.0)
    [[ "${cache_mode}" == no-cache ]] && build_args+=(--no-cache)
    build_args+=(.)
    printf 'Building the AtSubP2 application with one Podman stage at a time...\n'
    podman "${build_args[@]}"
}
...
run_existing_deployment() {
    ...
    case "${ACTION}" in
        start) printf 'Starting existing AtSubP2 containers without pulling or rebuilding...\n'; "${ENGINE}" compose --env-file deploy/docker.env "${COMPOSE_FILES[@]}" up -d --no-build ;;
        update)
        ...
        rebuild)
            printf 'Replacing this Compose project and rebuilding without cache. Job data and downloads are preserved.\n'
...
prompt_value BIOCLUSTER_REMOTE_SCRIPT 'Absolute HPC path to the AtSubP2 SLURM script'
...
prompt_value NEXT_PUBLIC_BASE_PATH 'Public URL base path' '/AtSubP2'

for key_default in PREDICTION_TIMEOUT_MS:0 PREDICTION_JOB_RETENTION_MS:2592000000 PREDICTION_MAX_ACTIVE_JOBS:10 PREDICTION_MAX_ACTIVE_JOBS_PER_CLIENT:2 MAX_REQUEST_BODY_BYTES:1048576 PREDICTION_MAX_SEQUENCES:10000 PREDICTION_MAX_RESIDUES:1000000 PREDICTION_MAX_SEQUENCE_LENGTH:20000 MAX_ACCESSION_COUNT:50 MAX_ACCESSION_LENGTH:64; do
