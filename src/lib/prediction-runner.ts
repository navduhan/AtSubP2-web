...
const RESULT_FILES = new Set([
  'AtSubP2_single_vs_dual_localizations.txt',
  'AtSubP2_single_class_localizations.txt',
  'AtSubP2_dual_class_localizations.txt',
  'AtSubP2_membrane_classification.txt',
]);
...
async function runLocal(request: PredictionRequest, remoteError: string): Promise<PredictionRun> {
  ...
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), 'atsubp2-'));
