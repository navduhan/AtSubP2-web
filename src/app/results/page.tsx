...
async function fetchJob(job: JobBookmark) {
  const response = await fetch(withBasePath(`/api/predict?jobId=${encodeURIComponent(job.jobId)}`), { cache: 'no-store', headers: { 'X-AtSubP2-Job-Token': job.jobToken } });
...
const levelMeta = (fileName: string) => {
  ...
  if (fileName.includes('single_class')) return { level: 'Level II', title: 'Single-localization class', note: '12 cellular compartments' };
  if (fileName.includes('dual_class')) return { level: 'Level III', title: 'Dual-localization class', note: '9 compartment pairs' };
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (!savedJob) {
        const saved = localStorage.getItem('atsubp2_last_results');
        if (saved) try {
          const parsed = JSON.parse(saved) as StoredResults;
          setData(parsed);
          setActiveLevel(sortByLevel(Object.keys(parsed.results || {}))[0] || '');
        } catch (error: unknown) { console.error('Unable to read stored AtSubP2 results.', error); }
        setLoaded(true);
        return;
      }
      ...
      void (async () => {
        try {
          ...
          localStorage.setItem('atsubp2_last_results', JSON.stringify(result));
          setData(result);
  ...
  const downloadActive = () => {
    if (activeRows.length) saveTextFile(rowsToTsv(activeRows), activeLevel || 'AtSubP2_results.txt');
  };

  const downloadAll = () => {
    const content = resultFiles.map((file) => `# ${levelMeta(file).level}: ${levelMeta(file).title}\n${rowsToTsv(data?.results?.[file] || [])}`).join('\n\n');
  ...
  if (!data || resultFiles.length === 0) {
    return (
      <div className="container mx-auto max-w-5xl py-10 sm:py-16">
        <section className="relative overflow-hidden rounded-[2rem] border border-[#d4e5e0] bg-[#f7f9f8] px-6 py-16 text-center sm:px-12">
  ...
  return (
    <div className="container mx-auto max-w-7xl space-y-6 py-5 sm:py-8">
      ...
      {bookmark && <section className="rounded-2xl border border-[#c8e2dc] bg-[#f6faf9] p-4 sm:flex sm:items-center sm:gap-4" aria-label="Saved results link"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-[#218870]"><Bookmark className="h-4 w-4" /></span><div className="mt-3 min-w-0 flex-1 sm:mt-0"><p className="text-xs font-bold text-[#082b3b]">Bookmark this private results link</p><p className="mt-1 truncate font-mono text-[10px] text-slate-500" title={resultBookmarkUrl}>{resultBookmarkUrl}</p><p className="mt-1 text-[10px] text-slate-500">Anyone with this link can view the result until it expires. Do not share it publicly.</p></div><button type="button" onClick={() => void navigator.clipboard.writeText(resultBookmarkUrl).then(() => { setLinkCopied(true); window.setTimeout(() => setLinkCopied(false), 1800); })} className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-[#082b3b] px-4 py-2.5 text-xs font-bold text-white sm:mt-0">{linkCopied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{linkCopied ? 'Copied' : 'Copy link'}</button></section>}
        ...
        <nav className="rounded-[1.5rem] border border-slate-200 bg-[#f7f9f8] p-3" aria-label="Prediction levels">
          ...
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            {resultFiles.map((fileName) => {
              ...
              return <button key={fileName} type="button" onClick={() => setActiveLevel(fileName)} className={`w-full rounded-xl border p-3 text-left transition ${isActive ? 'border-[#218870] bg-[#218870] text-white shadow-sm' : 'border-transparent bg-white text-slate-700 hover:border-[#b9d8d0]'}`}><span className={`text-[10px] font-black uppercase tracking-[0.15em] ${isActive ? 'text-[#bfe9df]' : 'text-[#218870]'}`}>{meta.level}</span><span className="mt-1 block text-sm font-bold leading-5">{meta.title}</span><span className={`mt-1 block text-[11px] ${isActive ? 'text-slate-200' : 'text-slate-500'}`}>{meta.note}</span></button>;
            })}
          </div>
        </nav>
        ...
        <section className="min-w-0 overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(8,43,59,0.06)]">
          ...
          <div className="results-table-scroll overflow-x-scroll">
            <table className="w-full min-w-[680px] text-left text-xs">
              <thead className="bg-[#edf7f4] text-[#082b3b]"><tr>{activeRows.length > 0 && Object.keys(activeRows[0]).map((header) => <th key={header} className="border-b border-[#d3e5e0] px-5 py-3.5 font-black uppercase tracking-[0.08em]">{header}</th>)}</tr></thead>
              <tbody className="divide-y divide-slate-100 font-mono">
                {activeRows.map((row, rowIndex) => <tr key={rowIndex} className="transition hover:bg-[#fafcfb]">{Object.entries(row).map(([key, value]) => <td key={key} className="whitespace-nowrap px-5 py-3.5 text-slate-700">{key.toLowerCase().includes('prediction') ? <span className="rounded-full border border-[#b9ddd4] bg-[#eef9f6] px-2.5 py-1 font-sans font-bold text-[#218870]">{String(value ?? '')}</span> : <span>{String(value ?? '')}</span>}</td>)}</tr>)}
