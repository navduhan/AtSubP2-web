...
export default function ResultsPage() {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (saved) {
        try {
          ...
          console.error('Unable to read stored AtSubP2 results.', error);
        }
      }
      setLoaded(true);
    }, 0);
  ...
  if (!loaded) return <div className="container mx-auto min-h-[480px] max-w-7xl py-8" aria-live="polite" />;
    ...
    return (
      <div className="container mx-auto max-w-5xl py-10 sm:py-16">
        <section className="relative overflow-hidden rounded-[2rem] border border-[#d4e5e0] bg-[#f7f9f8] px-6 py-16 text-center sm:px-12">
          ...
          <p className="relative mx-auto mt-4 max-w-xl text-sm leading-6 text-slate-600">Submit Arabidopsis protein sequences on the Prediction page. Completed results will open here automatically and remain available in this browser.</p>
  ...
  return (
    <div className="container mx-auto max-w-7xl space-y-6 py-5 sm:py-8">
      ...
      <section className="grid gap-3 sm:grid-cols-3" aria-label="Run summary">
        ...
        <div className={`rounded-2xl border p-5 ${data.executionMode === 'slurm' ? 'border-[#c8e2dc] bg-[#eef8f5]' : 'border-amber-200 bg-amber-50'}`}><p className={`text-[10px] font-black uppercase tracking-[0.16em] ${data.executionMode === 'slurm' ? 'text-[#218870]' : 'text-amber-700'}`}>Executor</p><p className={`mt-2 font-serif text-3xl font-semibold ${data.executionMode === 'slurm' ? 'text-[#218870]' : 'text-amber-800'}`}>{data.executionMode === 'slurm' ? 'SLURM' : data.executionMode === 'local' ? 'Local fallback' : 'Unverified'}</p><p className="mt-1 text-xs text-slate-600">{data.clusterJobId ? `Cluster job ${data.clusterJobId}` : data.executionMode ? 'Real model output' : 'Legacy stored result'}</p></div>
      </section>
