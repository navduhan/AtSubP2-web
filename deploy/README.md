# AtSubP-2.0 VM deployment

This deployment runs the Next.js application behind an unprivileged Nginx gateway on VM loopback port 3210. Prediction inputs are transferred over pinned-key SSH/SFTP to the configured cluster, submitted with `sbatch --parsable --wait`, and the real result files are retrieved over SFTP.
...
Private results are retained for 30 days by default (`PREDICTION_JOB_RETENTION_MS=2592000000`). Install the included cleanup command in the rootless Podman user's crontab, replacing `/absolute/path/to/AtSubP2-web` with the cloned repository path:

```cron
17 3 * * * /absolute/path/to/AtSubP2-web/deploy/prune-expired-jobs.sh
```
